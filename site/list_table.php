...
<div class="container">
    <div class="row">
        ...
        <div class="col-md-3  mb-2">
            <select class="form-select form-control"  name="sort_by_table_category" onchange="ChangeSortByTableCategory(this.value);">
              <option  value="" selected>Masa Kategorileri</option>
              <?php 
    ...
    <form  onclick="return false;" id="pagination-form">
        ...
        <input type="hidden" name="restart" value="GetTableList">
        <input type="hidden" name="search_key" id="hidden-search" value="">
        <input type="hidden" name="sort_by_name" id="hidden-search-name" value="">
        <input type="hidden" name="sort_by_table_count" id="hidden-search-count" value="">
        <input type="hidden" name="sort_by_table_category" id="hidden-search-category" value="">
    </form>
...
<script>
    ...
    function DeleteTable(TableId='')
    {
        if(TableId!='')
        {
            Swal.fire({
                title: 'Emin misiniz?',
                text: "Masayı silmek istediğinize emin misiniz ? ",
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        ...
                        data: 'ajax=true&build=DeleteTable&t_id=' + TableId,
    ...
    function ClearFilter()
    {
        ...
        $('input[name="sort_by_table_category"]').val('');
        ...
        document.querySelector('select[name="sort_by_table_category"]').selectedIndex=0;
        setTimeout(GetTableList, 10);
    ...
    function GetTableEditDataHtml(t_id=0)
    {
        $.ajax({
            type: "POST",
            dataType: 'json',
            url: '<?php echo apath; ?>' +'panel/important/ajax.php',
            data: 'ajax=true&build=GetTableEditDataHtml&t_id='+t_id,
            success: function(data) {
                var resMessageData = jQuery.parseJSON(data['responseMessage']);
    ...
    function ChangeSortByName(selectedValue) 
    {
        ...
        setTimeout(GetTableList, 25);

    }
    function ChangeSortByTableCount(selectedValue) 
    {
        $('input[name="sort_by_table_count"]').val(selectedValue);
        setTimeout(GetTableList, 125);

    }
    function ChangeSortByTableCategory(selectedValue) 
    {
        $('input[name="sort_by_table_category"]').val(selectedValue);
        $('input[name="page"]').val("1");
        setTimeout(GetTableList, 125);

    }
    function EditTable(t_id=0)
    {
        $.ajax({
            type: "POST",
            dataType: 'json',
            url: '<?php echo apath; ?>' +'panel/important/ajax.php',
            data: $("#table_edit_form").serialize(),
            success: function(data) {
                var resMessageData = jQuery.parseJSON(data['responseMessage']);
                autoSystemMessage(resMessageData);
                GetTableList();
    ...
    function GetTableList()
    {
        ...
        var sort_by_table_category = $('input[name="sort_by_table_category"]').val();
        $.ajax({
            type: "POST",
            dataType: 'json',
            url: '<?php echo apath; ?>' +'panel/important/ajax.php',
            data: 'ajax=true&build=GetAllTable&page='+pageValue+'&count_data='+countDataValue+'&size='+size+'&search_key='+search_key+'&sort_by_name='+sort_by_name + '&sort_by_table_count=' + sort_by_table_count+'&sort_by_table_category='+sort_by_table_category,
            success: function(data) {
                var resMessageData = jQuery.parseJSON(data['responseMessage']);
                $("#all-table").html(data['response']);
                $("#all-table").append(data['script']);
                ...
                //dataCount sıfır ise  sıfıra eşitledik. sonra pagination yaptık

                if(data['dataCount']>0) {
                    ChangePagePagination(pageValue,false);
                } else{
                    $('input[name="count_data"]').val("0");
                    ChangePagePagination(pageValue,false);
                }

                if(data['dataCount']==0) $("#all-table").append('<p style="margin-left:550px;">İçerik bulunamadı...</p>');
